        ...
        <ion-content :fullscreen="true">
            ...
            <div class="content">
                <form @submit.prevent="onSignIn">
                    <ion-list class="login-form-input">
                        ...
                        <ion-item class="input">
                            ...
                            <ion-note v-if="errors.email.required" color="danger">L'email est requis</ion-note>
                            <ion-note v-if="errors.email.email" color="danger">Email invalide</ion-note>
                        </ion-item>
                        <span class="input-label">Mot de passe</span>
                        <ion-item class="input">
                            <ion-input v-model="loginForm.motDePasse" type="password"></ion-input>
                            <ion-note v-if="errors.motDePasse.required" color="danger">Le mot de passe est
                                requis</ion-note>
                        </ion-item>
                    </ion-list>
                    <div class="login-buttons">
                        <ion-button class="custom-button" type="submit" expand="block" :disabled="!isFormValid">Je me
                            connecte</ion-button>
                        <ion-label class="forgot-password" @click="navigateToForgotPassword">Mot de passe oublié
                            ?</ion-label>
                    </div>
                </form>
                ...
                <div class="signup">
                    ...
                    <ion-button expand="block" @click="onSignUp" class="custom-button">Je m'inscris</ion-button>
                </div>
            </div>

            <ion-toast :isOpen="isErrorDisplayed" :message="loginError" :duration="3000"
                @didDismiss="() => isErrorDisplayed = false" position="top"></ion-toast>
        </ion-content>
    </ion-page>
</template>
...
import { IonPage, IonButton, IonContent, IonToast, IonList, IonItem, IonInput, IonLabel, IonNote } from '@ionic/vue';
...
const validateForm = () => {
    ...
    errors.value.email.email = !emailPattern.test(loginForm.value.email) && !errors.value.email.required;

    errors.value.motDePasse.required = !loginForm.value.motDePasse;

    if (errors.value.email.required || errors.value.email.email || errors.value.motDePasse.required) {
        isValid = false;
    }
...
const onSignIn = async () => {
    try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/utilisateurs/connexion`, {
            method: 'POST',
            headers: {
        ...
        loginError.value = 'Erreur lors de la connexion, veuillez vérifier votre réseau.';
        isErrorDisplayed.value = true;
