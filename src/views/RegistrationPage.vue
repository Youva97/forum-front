  ...
  <ion-content :fullscreen="true">
        ...
        <ion-list class="registrationForm">
          ...
          <ion-item>
            <ion-input
              ...
              id="nom"
            ></ion-input>
            <ion-note v-if="errors.nom.required" color="danger"
              >Nom est requis</ion-note
            >
            <ion-note v-if="errors.nom.minlength" color="danger"
              >Le nom doit avoir au moins 2 caractères</ion-note
            >
          </ion-item>
          ...
          <ion-item>
            <ion-input
              ...
              id="prenom"
            ></ion-input>
            <ion-note v-if="errors.prenom.required" color="danger"
              >Prénom est requis</ion-note
            >
            <ion-note v-if="errors.prenom.minlength" color="danger"
              >Le prénom doit avoir au moins 2 caractères</ion-note
            >
          </ion-item>
          ...
          <ion-item>
            <ion-input
              ...
              id="telephone"
            ></ion-input>
            <ion-note v-if="errors.telephone.required" color="danger"
              >Téléphone est requis</ion-note
            >
            <ion-note v-if="errors.telephone.pattern" color="danger"
              >Téléphone invalide</ion-note
            >
          </ion-item>
          ...
          <ion-item>
            ...
            <ion-note v-if="errors.email.required" color="danger"
              >Email est requis</ion-note
            >
            <ion-note v-if="errors.email.email" color="danger"
              >Email invalide</ion-note
            >
            <ion-note v-if="errors.email.uniqueEmail" color="danger"
              >Votre email est déjà utilisé</ion-note
            >
            <ion-note v-if="errors.email.uniqueCheckError" color="danger"
              >Nous n'arrivons pas à vérifier si votre email est déjà
              utilisé</ion-note
            >
          </ion-item>
          ...
          <ion-item>
            ...
            <ion-note v-if="errors.motDePasse.required" color="danger"
              >Mot de passe est requis</ion-note
            >
            <ion-note v-if="errors.motDePasse.minlength" color="danger"
              >Le mot de passe doit avoir au moins 8 caractères</ion-note
            >
          </ion-item>
        </ion-list>
    ...
    <div v-if="currentStep === 2">
      <div class="successContent">
        ...
        <p>
          Bienvenue sur l’application LEA ! Vous pourrez accéder à votre espace,
          mais également aux différents outils que fournit l’application.
        </p>
        <div class="validerBtn">
          <ion-button
            @click="goToUserSpace"
            expand="block"
            class="redirectToMySpaceButton"
            >Aller sur mon espace</ion-button
          >
        </div>
      </div>
    </div>
...
const validateFormStep1 = () => {
  ...
  if (
    !utilisateur.value.telephone ||
    !phonePattern.test(utilisateur.value.telephone)
  ) {
    errors.value.telephone.required = true;
    errors.value.telephone.pattern = !phonePattern.test(
      utilisateur.value.telephone
    );
  ...
  if (
    ...
    utilisateur.value.motDePasse.length < 8
  ) {
